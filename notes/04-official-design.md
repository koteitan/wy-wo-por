...
- **1 の再構成（`BlockReconstruction`）.** [OmegaY/Official/Recon/](../OmegaY/Official/Recon/) で、新しい列についての 2 つの主張に帰着した（`Recon.reconstructionHolds_of_rowLaw_chain`）。行の法則 `RowLawHolds`（新しい列で上の節点の行が `B(下の行, 親の行)`）と、親の鎖 `ChainHolds`（`u⁺` の格納された親が、`Q u` から格納された親をたどって届き、途中の節点の値が `v(u⁺)` 以上）である。新しい列の底の節点は証明した（`bottomHolds`）。2 つの主張は、474 個の fixture と、JS の標本の新しい節点約 220 万個で反例が無い。親の鎖は、さらに 2 つに分けた（`Recon.chainHolds_of_split`）。親が上の節点より低いこと（`ParentBelowHolds`）と、候補 `Q u` が親の列にないときの鎖（`CrossChainHolds`）である。候補が親と同じ列にあるときは証明した（`Q_eq_of_same_column`）。474 個の fixture の新しい節点 16085 個のうち、候補が親の列にないのは 1062 個だった。
