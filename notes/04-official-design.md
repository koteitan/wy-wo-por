...
- **未解決（§6）.** 分類がすべての展開で成り立つことは、証明していない。これが残る組合せの補題である。Lean では、この補題を仮定にした整礎性の定理を目標にする（§5）。
...
1. 脚の原子の系 $`\mathcal E_D`$ と、分類の判定を、Phyrion 氏の山の型の上で計算できる関数として定義する。いくつかの展開で判定を `#guard` で確かめる。
2. 分類を仮定にした整礎性の定理を示す。
   - 表現：狭義増加のラベルで、$`\mathcal E_D(M(s))`$ の原子がすべて成り立つもの。
   - 最初の表現：`initial_finite_graph`。
   - 降下：分類から `Splice.iterated_reservoirs` の仮定を作る。
