/-
Adapted from Phyrion, omega-Y-Well-Ordering-Lean, OmegaY/Expansion/ActualPhysicalEffectiveBarrier.lean,
...
private theorem shared_addend_of_column_le (hLast : 1 < last)
  ...
  have hURow : F.height u = F.height parent :=
    (build_marked_parent p.reduced_build hMarkers hUP hUMarked hRight).2
  have hZRow : F.height z = F.height parent :=
    (build_marked_parent p.reduced_build hMarkers hZP hZMarked hRight).2
  ...
  obtain ⟨pair⟩ := build_common_parent_marker_bands p.reduced_build hMarkers hUP hZP hZUpper
    hSame.le (hSame ▸ hHigher) hRight.le du dz hUSource hZSource hUPP hUPL hZPP hZPL hBetween hRefs
...
theorem shared_addend (hLast : 1 < last)
    (hValid : MountainValid result) (hSums : MountainSums result)
    (hUP : (Frame.ofMountain p.reduced).P u = some parent)
    (hZP : (Frame.ofMountain p.reduced).P z = some parent)
    (hUMarked : BucketMem p.marked u.1.val (Frame.ref u))
  ...
  · exact uPhysical.shared_addend_of_column_le zPhysical hLast hValid hSums hUP hZP hUMarked hZMarked hRight hZU
  · obtain ⟨addend, hZ, hU⟩ := zPhysical.shared_addend_of_column_le uPhysical hLast hValid hSums
      hZP hUP hZMarked hUMarked hRight hUZ
    exact ⟨addend, hU, hZ⟩

theorem compare_effective (hLast : 1 < last)
    (hValid : MountainValid result) (hSums : MountainSums result)
    (hUP : (Frame.ofMountain p.reduced).P u = some parent)
    (hZP : (Frame.ofMountain p.reduced).P z = some parent)
    (hUMarked : BucketMem p.marked u.1.val (Frame.ref u))
  ...
  obtain ⟨addend, hU, hZ⟩ := uPhysical.shared_addend zPhysical hLast hValid hSums hUP hZP hUMarked hZMarked hRight
  omega

end PhysicalMarkerRead
/-- The physical search now leaves only the two effective endpoint values
to compare. Source search, actual candidates, physical raw paths, and the
complete common fill-band cancellation are all derived inside the result. -/
theorem DynamicBlockState.physical_marker_effective_recognition
    ...
    ∃ (parentCopy : EffectiveCopyOccurrence p block start references parent ambient)
      ...
      ∃ (blocker : (Frame.ofMountain p.reduced).Node)
        ...
        (physical.cell.value ≤ blockerPhysical.cell.value ↔
          sourceCopy.read.outputCell.value ≤ blockerCopy.read.outputCell.value) ∧
        (sourceCopy.read.outputCell.value ≤ blockerCopy.read.outputCell.value →
          findParent ambient physical.outputRef = .ok parentPhysical.outputRef) := by
  obtain ⟨parentCopy, parentPhysical, hDirect | ⟨blocker, blockerCopy, blockerPhysical,
      hBlockerParent, hBlockerMarked, hSourceBarrier, recognize⟩⟩ :=
    s.physical_marker_recognition history hLast hSums physical hParent hMarked hRight hBefore hLeft
  · exact ⟨parentCopy, parentPhysical, Or.inl hDirect⟩
  · have hCompare := (physical.compare_effective blockerPhysical hLast s.ambient_valid hSums
      hParent hBlockerParent hMarked hBlockerMarked hRight).1
    exact ⟨parentCopy, parentPhysical, Or.inr ⟨blocker, blockerCopy, blockerPhysical,
      hBlockerParent, hBlockerMarked, hSourceBarrier, hCompare, fun h => recognize (hCompare.mpr h)⟩⟩
...
/-- The established source-blocker packet's actual event/depth-word
comparison can serve as the physical-marker comparison with no extra
value-transport claim: the two target comparisons are exactly equivalent. -/
theorem physical_effective_barrier (hLast : 1 < last)
    (hValid : MountainValid result) (hSums : MountainSums result)
    (hMarked : BucketMem p.marked source.1.val (Frame.ref source))
    (hRight : p.root.column < parent.1.val)
    (uPhysical : PhysicalMarkerRead packet.pair.uCopy)
    (zPhysical : PhysicalMarkerRead packet.pair.zCopy) :
    (uPhysical.cell.value ≤ zPhysical.cell.value ↔
      (Frame.ofMountain result).value packet.pair.u ≤ (Frame.ofMountain result).value packet.pair.z) ∧
    (uPhysical.cell.value < zPhysical.cell.value ↔
  ...
  have hZMarked := (build_common_parent_marker_iff p.reduced_build hMarkers packet.source_parent
    ...
    ((congrArg Ref.column hRootRef).trans_le hRight.le)).mp hMarked
  have hCompare := uPhysical.compare_effective zPhysical hLast hValid hSums
    packet.source_parent packet.source_last_parent hMarked hZMarked hRight
  have hUCell : (Frame.ofMountain result).cell packet.pair.u = packet.pair.uCopy.read.outputCell := by
    have hRead := Canonical.cellAt_of_frame_node result packet.pair.u
    rw [packet.pair.u_ref] at hRead
    exact Except.ok.inj (hRead.symm.trans packet.pair.uCopy.output_read)
  have hZCell : (Frame.ofMountain result).cell packet.pair.z = packet.pair.zCopy.read.outputCell := by
    have hRead := Canonical.cellAt_of_frame_node result packet.pair.z
    rw [packet.pair.z_ref] at hRead
    exact Except.ok.inj (hRead.symm.trans packet.pair.zCopy.output_read)
  simpa only [Frame.value, hUCell, hZCell] using hCompare

end SourceBlockerCopyPacket
end OmegaY.Expansion

#print axioms OmegaY.Expansion.PhysicalMarkerRead.shared_addend
#print axioms OmegaY.Expansion.PhysicalMarkerRead.compare_effective
#print axioms OmegaY.Expansion.DynamicBlockState.physical_marker_effective_recognition
#print axioms OmegaY.Expansion.SourceBlockerCopyPacket.physical_effective_barrier
